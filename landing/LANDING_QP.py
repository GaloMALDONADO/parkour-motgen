'''
Parkour Landing motion generation by hierarchical quadratic programming
'''
import os
import time
import sys
...
from hqp.standard_qp_solver import StandardQpSolver
from hqp.inv_dyn_formulation_util import InvDynFormulation
import hqp.plot_utils as plot_utils
...
#_ Robots Configuration
viewerName = 'Landing'
participantName = 'Lucas'
robotName = "Robot"
robotNode = 'world/'+robotName+'/'

# store the motion capture inverse kinematics results
def getMotionAnalysis(name):
    idxTraceur = mconf.traceurs_list.index(name)
...
# Get IK from Mocap
trial = getMotionAnalysis(participantName)
# Use trial 1 and modify position by 1 meter in medial lateral axis
r=1
...
IDXRF = robot.model.getFrameId('mtp_r')
IDXLF = robot.model.getFrameId('mtp_l')
